...
After returning to your app from Cash App, the component will be ready for the payment. If using `PlexySession` as your component's delegate, the payment will be handled automatically.
...
Set your `PlexySession` instance as Drop-in's delegate to let it handle the flow.
```
...
```
let component = CashAppPayComponent(paymentMethod: paymentMethod, context: plexyContext, configuration: configuration)
```

Set your `PlexySession` instance as the delegate to let it handle the flow.
```
component.delegate = self.plexySession
```
...
If you are using the default integration with the `/sessions` call, you will need to set `recurringProcessingModel` and `storePaymentMethodMode` properties (v70+) ([API Explorer](https://docs.plexy.com/api-explorer/Checkout/70/post/sessions#request-storePaymentMethodMode))
...
After the above configurations, on the default flow, `PlexySession` will make the required `/payments` call to authorize/store the payment.
