...
public final class PlexyActionComponent: ActionComponent, ActionHandlingComponent {
    ...
    public struct Configuration: Localizable {
        ...
        public struct Twint {
            ...
            public init(
                ...
                if !Self.isCallbackSchemeValid(callbackAppScheme) {
                    PlexyAssertion.assertionFailure(message: "Format of provided callbackAppScheme '\(callbackAppScheme)' is incorrect.")
                }
        ...
        public init(
            ...
            threeDS: PlexyActionComponent.Configuration.ThreeDS = .init(),
            twint: Twint? = nil
        ) {
    ...
    private func handle(_ action: ThreeDS2ChallengeAction) {
        guard let threeDS2Component = currentActionComponent as? ThreeDS2Component else {
            PlexyAssertion.assertionFailure( // swiftlint:disable:next line_length
    ...
    private func handle(_ action: TwintSDKAction) {
            ...
            guard let twintConfiguration = configuration.twint else {
                PlexyAssertion.assertionFailure(
                    message: "Twint action configuration instance must not be nil in order to use PlexyTwint")
                return
            }
