...
internal class ShareableVoucherView: UIView, Localizable {
    ...
    private func addVoucherView() {
        ...
        voucherCardView.plexy.anchor(inside: self)
    }
    ...
    private lazy var logoView: UIImageView = {
        ...
        imageView.plexy.round(using: model.style.logoCornerRounding)
    ...
    internal func createTopView() -> UIView {
        ...
        let textLabelWrapper = textLabel.plexy.wrapped(
    ...
    private func createSeparator() -> UIView {
        let view = UIView()
        view.backgroundColor = UIColor.Plexy.lightGray
    ...
    private func createKeyValueView(key: String, value: String, identifier: String) -> UIView {
        ...
        let containerView = stackView.plexy.wrapped(
            with: UIEdgeInsets(top: 0, left: 16, bottom: 0, right: 16)
        )
    ...
    private func createLabel(with style: TextStyle, text: String?, identifier: String) -> UILabel {
        ...
        label.accessibilityIdentifier = ViewIdentifierBuilder.build(scopeInstance: "plexy.voucher", postfix: identifier)
    ...
    internal struct Model {
        ...
        internal struct Style {
            ...
            internal var amount = TextStyle(
                font: UIFont.preferredFont(forTextStyle: .callout).plexy.font(with: .bold),
                color: UIColor.Plexy.componentLabel,
                textAlignment: .center
            )
            
            internal var codeText = TextStyle(
                font: UIFont.preferredFont(forTextStyle: .title1).plexy.font(with: .bold),
                color: UIColor.Plexy.componentLabel,
                textAlignment: .center
            )
            
            internal var fieldValueText = TextStyle(
                font: UIFont.preferredFont(forTextStyle: .footnote).plexy.font(with: .semibold),
                color: UIColor.Plexy.componentLabel,
                textAlignment: .center
            )
