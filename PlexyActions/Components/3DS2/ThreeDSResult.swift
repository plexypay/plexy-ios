...
public struct ThreeDSResult: Decodable {
    ...
    internal init(
        ...
        self.payload = try PlexyCoder.encode(payload).base64EncodedString()
    }
    ...
    internal func withDelegatedAuthenticationSDKOutput(delegatedAuthenticationSDKOutput: String?) throws -> ThreeDSResult {
        let oldPayload: Payload = try PlexyCoder.decodeBase64(payload)
        ...
        return try .init(payload: PlexyCoder.encode(newPayload).base64EncodedString())
    }
