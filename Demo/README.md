...
The [`Common`](Common/) folder contains all basic code necessary to handle UI, Plexy checkout's state, and network calls.
...
Run the UIKit Demo by selecting `PlexyUIHost` as the target in Xcode.

### SwiftUI Demo

This is the basic UI for SwiftUI integration.
Run the SwiftUI Demo it by selecting `PlexySwiftUIHost` as the target in Xcode.
