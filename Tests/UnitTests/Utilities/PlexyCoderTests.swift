...
class PlexyCoderTests: XCTestCase {
    
    func testDecodeWithString() throws {
        let sampleObject = try PlexyCoder.decode(sampleObjectRawString) as SampleObject
    ...
    func testDecodeWithData() throws {
        ...
        let sampleObject = try PlexyCoder.decode(data) as SampleObject
    ...
    func testEncodeToString() throws {
        let encodedString = try PlexyCoder.encode(sampleObject) as String
        
        XCTAssertEqual(encodedString, sampleObjectRawString)
    }
    
    func testEncodeToData() throws {
        let encodedData = try PlexyCoder.encode(sampleObject) as Data
