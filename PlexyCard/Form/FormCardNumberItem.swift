...
internal final class FormCardNumberItem: FormTextItem, PlexyObserver {
    ...
    @PlexyObservable("") internal var binValue: String
    
    /// Initial brand set after detection before any user interaction
    @PlexyObservable(nil) internal private(set) var initialBrand: CardBrand?
    
    /// Brand selected in dual branded cards, set after user selection.
    @PlexyObservable(nil) internal private(set) var selectedDualBrand: CardBrand?
    
    /// Detected brand logo(s) for the entered bin.
    @PlexyObservable([]) internal private(set) var detectedBrandLogos: [FormCardLogosItem.CardTypeLogo]
    
    /// Determines whether the item is currently the focused one (first responder).
    @PlexyObservable(false) internal var isActive
    ...
    private func valueDidChange(_ value: String) {
        cardNumberFormatter.cardType = supportedCardTypes.plexy.type(forCardNumber: value)
        updateBINIfNeeded()
    }
