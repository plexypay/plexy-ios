...
public final class ListItemView: UIView, AnyFormItemView {
    ...
    private var imageLoadingTask: PlexyCancellable? {
    ...
    public var item: ListItem? {
        didSet {
            ...
            if let style = item?.style, oldValue?.style != style {
                ...
                titleLabel.plexy.apply(style.title)
                subtitleLabel.plexy.apply(style.subtitle)
                
                if let trailingTextLabel = trailingView as? UILabel {
                    trailingTextLabel.plexy.apply(style.trailingText)
                }
            }
        }
    }
    ...
    override public func layoutSubviews() {
        ...
        guard item?.icon?.canBeModified == true else {
            return imageView.plexy.round(using: .none)
        }

        imageView.plexy.round(using: item?.style.image.cornerRounding ?? .fixed(8))
    }
    ...
    override public func traitCollectionDidChange(_ previousTraitCollection: UITraitCollection?) {
        ...
        imageView.layer.borderColor = item?.style.image.borderColor?.cgColor ?? UIColor.Plexy.componentSeparator.cgColor
    }
