...
internal final class ListHeaderView: UITableViewHeaderFooterView {
    ...
    private func updateItem() {
        ...
        titleLabel.plexy.apply(item.style.title)
        titleLabel.accessibilityIdentifier = ViewIdentifierBuilder.build(
            scopeInstance: "Plexy.ListHeaderView.\(item.title)",
            postfix: "titleLabel"
        )
        titleLabel.text = item.title.uppercased()
        
        trailingButton.plexy.apply(item.style.trailingButton)
    ...
    private func updateTrailingButtonTitle(with item: ListSectionHeader?) {
        guard let item = headerItem else { return }
        let localizedEdit = Bundle.Plexy.localizedEditCopy
        let localizedDone = Bundle.Plexy.localizedDoneCopy
    ...
    private func configureConstraints() {
        ...
        stackView.plexy.anchor(inside: self.layoutMarginsGuide)
        backgroundView?.plexy.anchor(inside: self)
    }
