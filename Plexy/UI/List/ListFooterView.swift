...
internal final class ListFooterView: UIView {
    ...
    internal init(title: String, style: ListSectionFooterStyle) {
        ...
        stackView.plexy.anchor(inside: self)
    ...
    private lazy var titleContainerView: UIView = {
        titleBackgroundView.plexy.wrapped(with: .init(top: 6, left: 16, bottom: 12, right: 16))
    }()
    ...
    private lazy var titleLabel: UILabel = {
        ...
        titleLabel.accessibilityIdentifier = ViewIdentifierBuilder.build(
            scopeInstance: "Plexy.ListFooterView.\(title)",
            postfix: "titleLabel"
        )
    ...
    private lazy var titleBackgroundView: UIView = {
        let view = titleLabel.plexy.wrapped(with: .init(top: 8, left: 16, bottom: 8, right: 16))
        view.backgroundColor = style.title.backgroundColor
        view.plexy.round(using: style.title.cornerRounding)
        return view
    }()
