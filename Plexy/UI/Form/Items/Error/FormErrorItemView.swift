...
internal final class FormErrorItemView: FormItemView<FormErrorItem> {
    ...
    internal required init(item: FormErrorItem) {
        ...
        containerView.plexy.anchor(inside: layoutMarginsGuide, with: UIEdgeInsets(top: 8, left: 0, bottom: 8, right: 0))
        containerView.backgroundColor = item.style.backgroundColor
        containerView.plexy.round(using: item.style.cornerRounding)
    ...
    private lazy var containerView: UIView = {
        ...
        return stackView.plexy.wrapped(with: UIEdgeInsets(top: 8, left: 16, bottom: 8, right: 16))
    }()
