...
/// Adds helper functionality to any `UIView` instance through the `plexy` property.
@_spi(PlexyInternal)
extension PlexyScope where Base: UIView {
    ...
    private func hideWithAnimation(
        ...
        let context = KeyFrameAnimationContext(
            ...
            completion: { [weak base] _ in
                base?.isHidden = hidden
                base?.alpha = hidden ? 0 : 1
                base?.plexy.updatePreferredContentSize()
            }
        )
