...
public struct PostalAddress: Equatable, Encodable {
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        let houseNumberOrNameValue = [houseNumberOrName, apartment]
            .compactMap { $0?.plexy.nilIfEmpty }
            .joined(separator: " ")
            .plexy.nilIfEmpty

        try container.encode(city.plexy.nilIfEmpty ?? PostalAddress.invalidValue, forKey: .city)
        try container.encode(country.plexy.nilIfEmpty ?? PostalAddress.invalidCountry, forKey: .country)
        try container.encode(houseNumberOrNameValue ?? PostalAddress.invalidValue, forKey: .houseNumberOrName)
        try container.encode(postalCode.plexy.nilIfEmpty ?? PostalAddress.invalidValue, forKey: .postalCode)
        try container.encode(stateOrProvince.plexy.nilIfEmpty ?? PostalAddress.invalidValue, forKey: .stateOrProvince)
        try container.encode(street.plexy.nilIfEmpty ?? PostalAddress.invalidValue, forKey: .street)
    }
    ...
    public static func == (lhs: PostalAddress, rhs: PostalAddress) -> Bool {
        ...
        .map { $0?.trimmingCharacters(in: .whitespaces).plexy.nilIfEmpty }
