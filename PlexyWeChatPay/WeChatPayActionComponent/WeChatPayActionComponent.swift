...
import Foundation
...
#if !targetEnvironment(simulator) && canImport(PlexyWeChatPayInternal)

    import PlexyWeChatPayInternal
    ...
    public final class WeChatPaySDKActionComponent: NSObject, AnyWeChatPaySDKActionComponent {
        ...
        private static let universalLink = "https://www.plexy.com/"

        @_spi(PlexyInternal)
        public var context: PlexyContext
        ...
        public init(context: PlexyContext) {
        ...
        private static func assertWeChatPayAppSchemeConfigured() {
            guard Bundle.main.plexy.isSchemeConfigured("weixin") else {
                return PlexyAssertion.assertionFailure(message: """
    ...
    extension WeChatPaySDKActionComponent: WXApiDelegate {
        ...
        public func onResp(_ resp: BaseResp) {
            guard let currentlyHandledAction else {
                return PlexyAssertion.assertionFailure(message: "no WeChatPaySDKAction were handled")
            }
    ...
    public final class WeChatPaySDKActionComponent: NSObject, AnyWeChatPaySDKActionComponent {
        ...
        public func handle(_ action: WeChatPaySDKAction) {
            PlexyAssertion.assertionFailure(message: "WeChatPaySDKActionComponent can only work on a real device.")
        }
