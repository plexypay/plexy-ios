...
> Important: If you are using multiple Plexy Components or the Drop-in, all usages of that localization key will be updated with the value you specify to override the default localization.
...
1. Add the `plexy.submitButton.formatted` key to your catalog.
...
- English, in the `en.lproj/Localizable.strings` file:
  ...
  ```strings
  "plexy.submitButton.formatted" = "Subscribe for %@";
  ```

- Spanish, in the `es.lproj/Localizable.strings` file:
  ```strings
  "plexy.submitButton.formatted" = "Suscríbete por %@";
  ```
...
In the following example, the SDK looks for the key `plexy_submitButton_formatted` in the `YOUR_LOCALIZATION_FILE.strings` (or `YOUR_LOCALIZATION_FILE.xcstrings` catalog) file within a custom bundle.
...
// Assuming 'configuration' is an instance of your Plexy component's configuration
