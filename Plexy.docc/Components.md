...
All Components need an ``PlexyContext``. An instance of ``PlexyContext`` wraps your client key, environment, payment, analytics configuration and so on.
...
var context: PlexyContext {
    ...
    return PlexyContext(apiContext: apiContext,
                        payment: payment,
...
Initialize a choosen component class and set the ``PlexySession`` instance as the `delegate` and `partialPaymentDelegate` (if needed) of the component instance.
...
Actions are handled by the ``PlexySession``.
