...
TEMP_PROJECT_PATH="$TEMP_PROJECT_FOLDER/Plexy" # Quoted for safety
FRAMEWORK_NAME=Plexy
DOCS_ROOT=docs
...
rsync -r Plexy "$TEMP_PROJECT_PATH/Sources/$FRAMEWORK_NAME"
rsync -r PlexyActions "$TEMP_PROJECT_PATH/Sources/$FRAMEWORK_NAME"
rsync -r PlexyComponents "$TEMP_PROJECT_PATH/Sources/$FRAMEWORK_NAME"
rsync -r PlexyEncryption "$TEMP_PROJECT_PATH/Sources/$FRAMEWORK_NAME"
rsync -r PlexyCard "$TEMP_PROJECT_PATH/Sources/$FRAMEWORK_NAME"
rsync -r PlexyDropIn "$TEMP_PROJECT_PATH/Sources/$FRAMEWORK_NAME"
rsync -r PlexySession "$TEMP_PROJECT_PATH/Sources/$FRAMEWORK_NAME"
rsync -r PlexyWeChatPay "$TEMP_PROJECT_PATH/Sources/$FRAMEWORK_NAME"
rsync -r PlexySwiftUI "$TEMP_PROJECT_PATH/Sources/$FRAMEWORK_NAME"
rsync -r PlexyCashAppPay "$TEMP_PROJECT_PATH/Sources/$FRAMEWORK_NAME"
rsync -r PlexyTwint "$TEMP_PROJECT_PATH/Sources/$FRAMEWORK_NAME"

# Copy the Plexy.docc folder to the temp package source folder
cp -a "$FRAMEWORK_NAME.docc" "$TEMP_PROJECT_PATH/Sources/$FRAMEWORK_NAME/$FRAMEWORK_NAME.docc"

# Rename PlexyActions/Utilities/BundleSPMExtension.swift to PlexyActions/Utilities/ActionsBundleSPMExtension.swift
mv "$TEMP_PROJECT_PATH/Sources/$FRAMEWORK_NAME/PlexyActions/Utilities/BundleSPMExtension.swift" \
"$TEMP_PROJECT_PATH/Sources/$FRAMEWORK_NAME/PlexyActions/Utilities/ActionsBundleSPMExtension.swift"

# Rename Plexy/Utilities/BundleSPMExtension.swift to Plexy/Utilities/CoreBundleSPMExtension.swift
mv "$TEMP_PROJECT_PATH/Sources/$FRAMEWORK_NAME/Plexy/Utilities/BundleSPMExtension.swift" \
"$TEMP_PROJECT_PATH/Sources/$FRAMEWORK_NAME/Plexy/Utilities/CoreBundleSPMExtension.swift"
...
let package = Package(
    ...
    targets: [
        ...
        .target(
            name: \"Plexy\",
            dependencies: [
                .product(name: \"PlexyNetworking\", package: \"PlexyNetworking\"),
                .product(name: \"Plexy3DS2\", package: \"Plexy3DS2\"),
                .product(name: \"PlexyWeChatPayInternal\", package: \"PlexyWeChatPayInternal\"),
            ...
            exclude: [
                \"Plexy/Info.plist\",
                \"PlexyActions/Info.plist\",
                \"PlexyComponents/Info.plist\",
                \"PlexyDropIn/Info.plist\",
                \"PlexyEncryption/Info.plist\",
                \"PlexySwiftUI/Info.plist\",
                \"PlexyWeChatPay/Info.plist\",
                \"PlexyCashAppPay/PlexyCashAppPay.docc\",
                \"PlexyCard/Info.plist\",
                \"PlexyCard/Utilities/Non SPM Bundle Extension\",
                \"PlexyActions/Utilities/Non SPM Bundle Extension\",
                \"Plexy/Utilities/Non SPM Bundle Extension\",
                \"PlexyTwint/PlexyTwint.docc\"
            ]
        ),
    ]
)
...
--hosting-base-path "/plexy-ios/$LATEST_VERSION"
...
echo "<head>
  <meta http-equiv=\"Refresh\" content=\"0; url='/plexy-ios/$LATEST_VERSION/documentation/plexy'\" />
</head>" > index.html
